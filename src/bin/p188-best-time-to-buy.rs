...
impl Solution {
    pub fn max_profit(k: i32, prices: Vec<i32>) -> i32 {
        ...
        for i in prices {
            if i < last_price {
                if last_price > min_price {
                    pairs.push((min_price, last_price));
                }
                min_price = i;
            }
            last_price = i;
        ...
        let max_earn = pairs.iter().fold(0, |s, &(buy, sell)| s + (sell - buy));
        let l = pairs.len();
        if (k as usize) < l {
        //     let mut skip_to_loss = Vec::<(usize, i32)>::new();

        //     let mut last_price = pairs[0].0;
        //     for (i, price) in pairs.into_iter().flat_map(|i| [i.0, i.1]).skip(1).enumerate() {
        //         skip_to_loss.push((i, -(price - last_price).abs()));
        //         last_price = price;
        //     }
        //     skip_to_loss.sort_by(|(_, x), (_, y)| y.cmp(x));
        //     max_earn + skip_to_loss
        //             .iter()
        //             .take(l - (k as usize))
        //             .fold(0, |s, &(_, x)| s + x)

            let a = &pairs[..];
            Self::dp(&mut HashMap::new(), &mut HashMap::new(), k, a, 0)
        } else {
            max_earn
        }
    }
    ...
    fn earn(
        ...
        if let Some(&ret) = cache.get(&(x, y)) {
            ret
        } else {
            let ret = pairs[y].1 - pairs[x].0;
            cache.insert((x, y), ret);
            ret
        }
    }
    ...
    fn dp(
        cache: &mut HashMap<(usize, usize), i32>,
        cache2: &mut HashMap<(i32, usize), i32>,
        k: i32,
        pairs: &[(i32, i32)],
        x: usize,
    ) -> i32 {
        ...
        if let Some(&m) = cache2.get(&(k, x)) {
            m
        } else {
            let mut max = 0;
            for i in x..pairs.len() {
                for j in i..pairs.len() {
                    let mut earn = Solution::earn(cache, pairs, i, j);
                    earn += Self::dp(cache, cache2, k - 1, pairs, j + 1);
                    if earn > max {
                        max = earn;
                    }
                }
            }
            cache2.insert((k, x), max);
            max
        }
    }
}
